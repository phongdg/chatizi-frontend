...
const CHANNELS = [
  ...
  },
  {
    ...
    // ghl_api_key is stripped by the backend on GET — hidden; uses global env var fallback
    fields: [
...
function buildValues(bot) {
  const init = {}
  CHANNELS.forEach((ch) => {
    if (ch.id === 'zalo') {
      init[ch.enabledKey] = (bot?.channels || []).includes('zalo') || !!bot?.zalo_oa_id
    } else if (ch.id === 'ghl') {
      init[ch.enabledKey] = !!bot?.ghl_location_id
    } else {
...
export default function IntegrationsTab({ bot, onUpdate }) {
  const [values, setValues] = useState(() => buildValues(bot))
  const [saved, setSaved] = useState({})

  useEffect(() => {
    if (bot) setValues(buildValues(bot))
  }, [bot])
